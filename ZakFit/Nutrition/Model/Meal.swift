...
struct Meal: Identifiable, Codable{
    ...
    init(from meal: MealData) throws {
        self.id = meal.id ?? .init()
        self.date = meal.date
        self.type = .init(rawValue: meal.type) ?? .breakfast
        self.foods = []
